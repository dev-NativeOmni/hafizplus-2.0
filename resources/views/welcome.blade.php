...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-zinc-50 text-zinc-800 dark:bg-[#09090b] dark:text-zinc-100 font-sans antialiased selection:bg-indigo-500 selection:text-white relative overflow-x-hidden min-h-screen transition-colors duration-200">
        ...
        <div class="relative z-10 flex flex-col min-h-screen" x-data="{ dark: document.documentElement.classList.contains('dark'), toggleTheme() { this.dark = !this.dark; if (this.dark) { document.documentElement.classList.add('dark'); localStorage.setItem('theme', 'dark'); } else { document.documentElement.classList.remove('dark'); localStorage.setItem('theme', 'light'); } } }">
                        ...
                        @if (Route::has('login'))
                            ...
                            @else
                                ...
                                @if (Route::has('register'))
                                    <a href="{{ route('register') }}" class="px-5 py-2 text-sm font-semibold rounded-xl text-white bg-indigo-600 hover:bg-indigo-500 border border-indigo-400/20 shadow-[0_4px_20px_rgba(99,102,241,0.25)] dark:shadow-[0_4px_20px_rgba(99,102,241,0.3)] transition-all duration-200 hover:scale-[1.02]">
                                        Mulai Sekarang
                                    </a>
                                @endif
                            @endauth
            ...
            <!-- Main Content -->
            <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-16 flex flex-col items-center gap-20">
                <!-- Hero Section -->
                <div class="text-center flex flex-col items-center gap-6 max-w-3xl">
                    <!-- Premium Badge -->
                    <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-zinc-200/50 dark:bg-white/5 border border-zinc-300/50 dark:border-white/10 backdrop-blur-md shadow-inner text-xs font-semibold text-indigo-600 dark:text-indigo-300">
                        <span class="flex h-2 w-2 relative">
                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                            <span class="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                        </span>
                        HafizPlus w/ Tailwind v4 & Theme Switcher
                    </div>

                    <!-- Main Heading -->
                    <h1 class="text-4xl sm:text-6xl font-bold tracking-tight text-zinc-900 dark:text-white leading-none">
                        Platform Tahfidz Qur'an <br>
                        <span class="font-serif italic text-gradient-purple-blue font-normal">Modern & Terstruktur</span>
                    </h1>

                    <!-- Description -->
                    <p class="text-lg text-zinc-650 dark:text-zinc-400 leading-relaxed max-w-xl">
                        Sistem pemantauan hafalan, murajaah, dan pelaporan progres santri secara real-time. Menghubungkan guru, orangtua, dan santri dalam satu ekosistem digital.
                    </p>

                    <!-- CTA Buttons -->
                    <div class="flex flex-col sm:flex-row items-center gap-4 mt-4">
                        @auth
                            <a href="{{ url('/dashboard') }}" class="px-8 py-3.5 text-base font-semibold rounded-xl text-white bg-indigo-600 hover:bg-indigo-500 border border-indigo-400/20 shadow-[0_4px_20px_rgba(99,102,241,0.3)] transition-all duration-200">
                                Masuk ke Dasbor Anda
                            </a>
                        @else
                            <a href="{{ route('login') }}" class="px-8 py-3.5 text-base font-semibold rounded-xl text-white bg-indigo-600 hover:bg-indigo-500 border border-indigo-400/20 shadow-[0_4px_20px_rgba(99,102,241,0.3)] transition-all duration-200">
                                Masuk Aplikasi
                            </a>
                            <a href="#features" class="bg-white/80 dark:bg-white/5 border border-zinc-200 dark:border-white/10 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-white/10 px-8 py-3.5 text-base font-semibold rounded-xl transition-colors duration-150 shadow-sm">
                                Pelajari Fitur
                            </a>
                        @endauth
                    </div>
                ...
                <!-- Mockup Showcase (Interactive App Preview) -->
                <div class="w-full max-w-5xl bg-white/80 dark:bg-zinc-900/45 rounded-3xl p-4 sm:p-6 border border-zinc-200 dark:border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.05)] dark:shadow-[0_30px_100px_rgba(0,0,0,0.8)] relative group overflow-hidden transition-all duration-200">
                    <!-- Glass shine overlay -->
                    ...
                    <!-- Mockup Header -->
                    <div class="flex items-center justify-between pb-4 border-b border-zinc-200/50 dark:border-white/5 mb-4">
                        <div class="flex items-center gap-2">
                            <span class="w-3 h-3 rounded-full bg-red-500/85"></span>
                            <span class="w-3 h-3 rounded-full bg-yellow-500/85"></span>
                            <span class="w-3 h-3 rounded-full bg-green-500/85"></span>
                        </div>
                        <div class="px-4 py-1 rounded-lg bg-zinc-100 dark:bg-white/5 border border-zinc-200 dark:border-white/5 text-[11px] text-zinc-500 font-mono">
                        ...
                        <div class="md:col-span-3 flex flex-col gap-4">
                            ...
                            <div class="bg-white dark:bg-white/5 border border-zinc-200 dark:border-white/5 p-4 rounded-xl flex-1 flex flex-col gap-3 min-h-[140px] shadow-sm">
                                ...
                                <div class="flex-1 flex items-end justify-between gap-2 pt-4 px-2">
                                    ...
                                    <div class="bg-indigo-600 border border-indigo-400 rounded-t w-full h-[90%] shadow-[0_0_12px_rgba(99,102,241,0.3)] dark:shadow-[0_0_12px_rgba(99,102,241,0.5)]"></div>
                                </div>
                            </div>
                        </div>
                    </div>
